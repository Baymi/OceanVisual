(function() {
    var OGCHelper = {};
    var loadXML=Engine.loadXML===undefined?Engine.Resource.fetchXML:Engine.loadXML;
    var loadArrayBuffer=Engine.loadArrayBuffer===undefined?Engine.Resource.fetchArrayBuffer:Engine.loadArrayBuffer;
    var loadImage=Engine.loadImage===undefined?Engine.Resource.fetchImage:Engine.loadImage;
    var intersectionRectangle = function(rectangle0, rectangle1) {
        ...
        if ((east <= west) || (south >= north)) {
            ...
            resultat = new Engine.Rectangle(west, south, east, north);
        }
        return resultat;
    };
    /**
    ...
    OGCHelper.CRS = [{
        ...
        tilingScheme: Engine.GeographicTilingScheme,
        ...
        ellipsoid: Engine.Ellipsoid.WGS84,
        firstAxeIsLatitude: false,
        tilingScheme: Engine.WebMercatorTilingScheme,
    ...
    OGCHelper.parser = function(description) {
        var resultat;
        description = Engine.defaultValue(description,
            Engine.defaultValue.EMPTY_OBJECT);
    ...
    OGCHelper.WMSParser.generate = function(description) {
        ...
        description = Engine.defaultValue(description,
            Engine.defaultValue.EMPTY_OBJECT);
        if (Engine.defined(description.url)) {
    ...
    OGCHelper.WMSParser.getMetaDatafromXML = function(xml, description) {
        if (!(xml instanceof XMLDocument)) {
            throw new Engine.DeveloperError('xml must be a XMLDocument');
        }
        // get version of wms 1.1.X or 1.3.X=> for 1.3 use firstAxe for order of
        // CRS
        if (!Engine.defined(description.layerName)) {
            throw new Engine.DeveloperError(
                'description.layerName is required.');
        }
        var resultat = {};
        var layerName = description.layerName;
        var maxLevel = Engine.defaultValue(description.maxLevel, 11);
        var version = undefined;
        resultat.heightMapWidth = Engine.defaultValue(description.heightMapWidth, 64);
        resultat.heightMapHeight = Engine.defaultValue(description.heightMapHeight, resultat.heightMapWidth);
        var requestedSize = {
            width: 64,
            height: 64
        };
        ...
        resultat.waterMask = Engine.defaultValue(description.waterMask, false);
        if (typeof(resultat.waterMask) !== "boolean") {
            resultat.waterMask = false;
        }
        resultat.offset = Engine.defaultValue(description.offset, 0);
        resultat.highest = Engine.defaultValue(description.highest, 12000);
        resultat.lowest = Engine.defaultValue(description.lowest, -500);
        var styleName = description.styleName;
        resultat.hasStyledImage = Engine.defaultValue(description.hasStyledImage, typeof(description.styleName) === "string");
        // get version
        ...
        if (!Engine.defined(resultat.formatImage)) {
            for (var j = 0; j < nodeFormats.length && !Engine.defined(resultat.formatArray); j++) {
        ...
        if (Engine.defined(resultat.formatArray) &&
            ...
            resultat.formatArray.terrainDataStructure = {
                heightScale: 1.0,//bildata
                heightOffset: 0,
                elementsPerHeight: 1,
                stride: 1,
                elementMultiplier: 256.0,
                isBigEndian: true,
                lowestEncodedHeight: -300000,
                highestEncodedHeight: 300000
            };
        } else {
            resultat.formatArray = undefined;
        }
        // a formatImage should always exist !!
        for (var j = 0; j < nodeFormats.length && !Engine.defined(resultat.formatImage); j++) {
        ...
        var layerNode;
        for (var m = 0; m < layerNodes.length && !Engine.defined(layerNode); m++) {
        ...
        if (Engine.defined(layerNode) && Engine.defined(version)) {
            var found = false;
            for (var n = 0; n < OGCHelper.CRS.length && !found; n++) {
                ...
                if (nodeBBox !== null) {
                    ...
                    var rectReference = new Engine.Rectangle(west, south, east, north);
                    resultat.getTileDataAvailable = function(x, y, level) {
                        ...
                        if (level < maxLevel) {
                            ...
                            retour = Engine.defined(scratchRectangle);
                        }
                        return retour;
                    };
                    found = true;
            ...
            resultat.ready = found &&
                (Engine.defined(resultat.formatImage) || Engine.defined(resultat.formatArray)) &&
                Engine.defined(version);
        }
    ...
    OGCHelper.TMSParser.parseXML = function(xml, description) {
        ...
        if (xml.querySelector("TileMapService") != null) {
            if (!Engine.defined(description.layerName)) {
                throw new Engine.DeveloperError('layerName is required.');
            }
            ...
            var promises = mapServiceNodes.map(function(elt) {
                ...
                return Engine.when(loadXML(url), function(xml) {
                    return OGCHelper.TMSParser.getMetaDatafromXML(xml, description);
                });
            });
            var promise = Engine.when.all(promises).then(function(tabResult) {
                var retour;
                for (var i = 0; i < tabResult.length && !Engine.defined(retour); i++) {
                    if (Engine.defined(tabResult[i])) {
                        retour = tabResult[i];
                    }
                }
                return retour;
    ...
    OGCHelper.TMSParser.getMetaDatafromXML = function(xml, description) {
        ...
        if (!Engine.defined(resultat.tileSets) || !Engine.defined(resultat.formatImage) || !Engine.defined(resultat.tilingScheme)) {
            resultat = undefined;
        } else {
            ...
            var limites = new Engine.Rectangle(minx, miny, maxx, maxy);
            resultat.getTileDataAvailable = function(x, y, level) {
                var rect = resultat.tilingScheme.tileXYToNativeRectangle(x, y, level);
                var scratchRectangle = intersectionRectangle(limites, rect);
                return Engine.defined(scratchRectangle) && level < maxLevel && level < tileSets.length;
            }
            resultat.ready = true;
        }
        return resultat;
    ...
    OGCHelper.WMTSParser.getMetaDatafromXML = function(xml, description) {
        ...
        resultat.heightMapWidth = Engine.defaultValue(description.heightMapWidth, 64);
        resultat.heightMapHeight = Engine.defaultValue(description.heightMapHeight, resultat.heightMapWidth);
        var maxLevel = Engine.defaultValue(description.maxLevel, 12);
        var proxy = description.proxy;
        var styleName = description.styleName;
        resultat.hasStyledImage = Engine.defaultValue(description.hasStyledImage, typeof(description.styleName) === "string");
        resultat.waterMask = Engine.defaultValue(description.waterMask, false);
        ...
        var layerNode;
        for (var i = 0; i < nodeIdentifiers.length && !Engine.defined(layerNode); i++) {
            if (layerName === nodeIdentifiers[i].textContent) {
                layerNode = nodeIdentifiers[i].parentNode;
            }
        }

        if (Engine.defined(layerNode)) {
            ...
            if (!Engine.defined(styleName) || styleName != selectedStyle) {
                styleName = Engine.defaultValue(defaultStyle, "");
            }

            //format
            var nodeFormats = [].slice.call(layerNode.querySelectorAll("Format"));
            for (var l = 0; l < OGCHelper.FormatImage.length &&
                !Engine.defined(formatImage); l++) {
        ...
        for (var a = 0; a < listTileMatrixSetLinkNode.length && !resultat.ready; a++) {
            ...
            for (var i = 0; i < nodeMatrixSetIds.length && !Engine.defined(tileMatrixSetNode); i++) {
            ...
            for (var n = 0; n < OGCHelper.CRS.length && !Engine.defined(CRSSelected); n++) {
                if (OGCHelper.CRS[n].SupportedCRS === supportedCRS) {
                    CRSSelected = OGCHelper.CRS[n];
                }
            }

            if (Engine.defined(CRSSelected)) {
                var tileSets;
    ...
    var GeoserverTerrainProvider = function GeoserverTerrainProvider(
        description) {
        if (!Engine.defined(description)) {
            throw new Engine.DeveloperError('description is required.');
        }
        var errorEvent = new Engine.Event();

        var credit = description.credit;
        if (typeof credit === 'string') {
            credit = new Engine.Credit(credit);
        }

        this.ready = false;
        this._readyPromise = Engine.when.defer();
        GeoserverTerrainProvider.heightScale = description.scale||1;
        Engine.defineProperties(this, {
            errorEvent: {
                get: function() {
                    return errorEvent;
                }
             ...
             * @memberof EngineTerrainProvider.prototype
    ...
    GeoserverTerrainProvider.arrayToHeightmapTerrainData = function(arrayBuffer, limitations, size, formatArray, hasWaterMask, childrenMask) {
        ...
        var heightBuffer = formatArray.postProcessArray(arrayBuffer, size, limitations.highest, limitations.lowest,
            limitations.offset);
        if (!Engine.defined(heightBuffer)) {
            throw new Engine.DeveloperError("no good size");
        }
        var optionsHeihtmapTerrainData = {
            ...
            structure: {
                ...formatArray.terrainDataStructure,
        ...
        return new Engine.HeightmapTerrainData(optionsHeihtmapTerrainData);
    };
    ...
    GeoserverTerrainProvider.imageToHeightmapTerrainData = function(image, limitations, size, hasWaterMask, childrenMask, hasStyledImage) {
        ...
        var dataPixels = Engine.getImagePixels(image, size.width, size.height);
    ...
    function TerrainParser(promise, provider) {
        Engine.when(promise, function(resultat) {
            console.log(resultat);
            if (Engine.defined(resultat) && (resultat.ready)) {
                resultat.levelZeroMaximumGeometricError = Engine.TerrainProvider.getEstimatedLevelZeroGeometricErrorForAHeightmap(
                    resultat.tilingScheme.ellipsoid, resultat.heightMapWidth,
                    resultat.tilingScheme.getNumberOfXTilesAtLevel(0));
                if (Engine.defined(resultat.URLtemplateImage)) {
                    resultat.getHeightmapTerrainDataImage = function(x, y, level) {
                        ...
                        if (!isNaN(x + y + level)) {
                            ...
                            if (Engine.defined(promise)) {
                                retour = Engine.when(promise, function(image) {
                                    ...
                                    return new Engine.HeightmapTerrainData({
                ...
                if (Engine.defined(resultat.URLtemplateArray)) {
                        ...
                        if (!isNaN(x + y + level)) {
                            ...
                            if (Engine.defined(promise)) {
                                retour = Engine.when(promise,
                                    ...
                                    function() {
                                        if (Engine.defined(resultat.getHeightmapTerrainDataImage)) {
                ...
                provider.requestTileGeometry = function(x, y, level) {
                    var retour;
                    if (Engine.defined(resultat.getHeightmapTerrainDataArray)) {
                        retour = resultat.getHeightmapTerrainDataArray(x, y, level);
                    } else if (Engine.defined(resultat.getHeightmapTerrainDataImage)) {
                        retour = resultat.getHeightmapTerrainDataImage(x, y, level);
                    }
                    return retour;
                }

                Engine.defineProperties(provider, {
                    tilingScheme: {
                        get: function() {
    ...
    GeoVis.GeoserverTerrainProvider = GeoserverTerrainProvider;
})();